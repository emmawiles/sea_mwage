...
stub.save <- "table_a4_jobs_cumulative_com_weights"
...
# Wages from jobs paying (<=$9.99) to (<=$18.99)
wge = 18 
...
for (j in outcomes) {
  ...
  # Get the estimates
  results.synth.temp <- EstimateSynth_common(j, cohort.series.synth.exclKing ,cohort.series.synth.exclKing[variable==j,],
...
saveRDS(results.synth.exclKing,paste0(path.to.data,stub.save,"_synth_exclKing_",mode.changes,".Rds"))			
...
results.coefs.best <- as.data.table(results.synth.exclKing)
...
table.to.save <- results.coefs.best[, c("variable","method","T","coefcum_text","pvalcum_text", "RMSE", "Obs")]

write.csv(rbind(as.matrix(table.to.save)), file = paste0(path.to.save,stub.save,".csv"))
