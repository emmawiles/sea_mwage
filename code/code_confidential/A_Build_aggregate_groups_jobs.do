...
if `do_build_analysis_data'==1 {
	...
	***** WAGERATE GROUP FOR THE WHOLE QUARTER *****	
	...
	replace wagegroup = 20 if wagegroup >=19 & wagegroup < 25
	replace wagegroup = 30 if wagegroup >=25 & wagegroup < 40
	...
	* Add firm identifiers
	...
	sort acct time
	save "${path_analysis_data}establishment_analysis_group_Spring_2019_`sample_size'.dta", replace
}	
...
if `do_sumstat_analysis_DiD'==1 | `do_sumstat_analysis_Synth'==1 {
	...
	* load the data	
	use using "${path_analysis_data}establishment_analysis_group_Spring_2019_`sample_size'.dta"
	...
	** Save data for synthetic control region analysis ****
	...
	di _n "Synth data prep -- `I' "
		...
		* COLLAPSE TO Save time series for each 
		collapse (sum) payroll_flow payroll_beg hours_flow hours_beg wages nworkers_beg N = operating (max) industry  , by(region puma_id yearquarter wagegroup)	
		...
		*** CREATE DATASETS
		...
		save "${path_collapsed_data}\\`Prefix'synth_bin_group_`I'.dta", replace
		restore	
		...
		preserve
			...
			* Treatment periods (Seattle minimum wage)					
			...
			save "${path_collapsed_data}\\`Prefix'synth_cumulative_groups_`I'.dta", replace
