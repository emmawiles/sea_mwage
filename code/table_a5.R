...
############################
# Run senstivity of synethetic control removing the 4 quarters prior to MW: 2013.3, 2013.4, 2014.1, 2014.2
Sens_less4 =1
...
if (Sens_less4 == 1) {
 cohort.series <- cohort.series[yearquarter != 20142, ]
 cohort.series <- cohort.series[yearquarter != 20141, ]
 	cohort.series <- cohort.series[yearquarter != 20134, ]
 	cohort.series <- cohort.series[yearquarter != 20133, ]
	}
...
results<- results.synth.exclKing %>% select(c("T","variable", "Obs", "RMSE", "coefcum", "pval.cum"))

results$sig[results$pval.cum > 0.1] <- ""
results$sig[results$pval.cum <= 0.1] <- "*"
results$sig[results$pval.cum <= 0.05] <- "**"
results$sig[results$pval.cum <= 0.025] <- "***"

write.csv(results, file = paste0(path.to.save,"table_a5_minus4q.csv"))
