...
#wge = 18:  Wages -- from jobs paying (<=$9.99) to (<=$18.99)
...
#### READ in DID FILE FOR SEATTLE AVERAGES
...
# Eliminate PUMAs codes with too few observations
...
cohort.series[grepl("nworkers",variable)==TRUE, stub_N := "L4_cum_nworkers"]
...
saveRDS(weights.synth.exclKing,paste0(path.to.save,stub.save,"_synth_weights_exclKing_",mode.changes,".Rds")) #important
...
saveRDS(bootstrap.synth.exclKing.Weights,paste0(path.to.save,stub.save,"_synth_bootstrapWeights_exclKing_",mode.changes,".Rds"))
...
for (t in seq(1,9)) {
  cohort.series.bai.exclKing[ , paste0("treated",t) := as.integer(region0 == 1 & R == t)]		
}
...
# Save results
saveRDS(results.bai.exclKing,paste0(path.to.save,stub.save,"_bai_exclKing_",mode.changes,".Rds"))
saveRDS(Vcov.bai.exclKing,paste0(path.to.save,stub.save,"_bai_Vcov_exclKing_",mode.changes,".Rds"))
saveRDS(factors.bai.exclKing,paste0(path.to.save,stub.save,"_bai_factors_exclKing_",mode.changes,".Rds"))
saveRDS(weights.bai.exclKing,paste0(path.to.save,stub.save,"_bai_weights_exclKing_",mode.changes,".Rds"))
...
### the line below ensuring this RDS is a datatable is newis new
